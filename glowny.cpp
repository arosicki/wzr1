...
#include "grafika.h"
using namespace std;
FILE *f = fopen("wzr_.txt", "w");    // plik do zapisu informacji testowych
...
void PoczatekInterakcji()
{
	...
	multi_reciv = new multicast_net("224.12.12.75", 10075);      // obiekt do odbioru ramek sieciowych
	multi_send = new multicast_net("224.12.12.75", 10075);       // obiekt do wysy³ania ramek
...
void Cykl_WS()
{
	...
	{                                   // nale¿y na nowo obliczyæ œredni czas cyklu fDt
		...
		sprintf(text, "WZR-lab 2024/25 temat 1, wersja b (%0.0f fps  %0.2fms)  ", fFps, 1000.0 / fFps);
	...
	int       nCmdShow)
{
	...
	static char nazwa_klasy[] = "KlasaGlowna";
	...
	okno = CreateWindow(nazwa_klasy, "WZR-lab 2024/25 temat 1, wersja b", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
		100, 50, 1000, 760, NULL, NULL, hInstance, NULL);
